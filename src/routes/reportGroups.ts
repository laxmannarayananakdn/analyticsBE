...
import {
  ...
  getUserReportGroups,
  setUserReportGroups,
} from '../services/ReportGroupService.js';
...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (!group) return res.status(404).json({ error: 'Report group not found' });
    res.json({
      reportGroupId: group.Report_Group_ID,
      groupName: group.Group_Name,
      groupDescription: group.Group_Description,
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (error.message === 'Report group not found') return res.status(404).json({ error: error.message });
    res.status(500).json({ error: error.message || 'Internal server error' });
  }
});
