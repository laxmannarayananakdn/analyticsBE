...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    if (curriculum_id) {
      ...
      `;
      const curriculumResult = await executeQuery<{ code: string; description: string }>(curriculumQuery, { curriculum_id });
    ...
    const insertResult = await executeQuery<any>(insertQuery, {
      country,
      school_name,
      client_id,
      client_secret,
    ...
    const selectResult = await executeQuery<any>(selectQuery, { id: newId });
...
router.put('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const updateResult = await executeQuery<any>(query, params);
    ...
    const selectResult = await executeQuery<any>(selectQuery, { id });
...
router.delete('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const deleteResult = await executeQuery<any>(deleteQuery, { id });
 ...
 */
router.get('/domain-url', async (req: Request, res: Response) => {
  try {
    const query = `
    ...
    const result = await executeQuery<{ domain_url: string }>(query);
