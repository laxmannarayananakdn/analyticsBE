 ...
 */
router.get('/sync-stream/:endpoint', loadManageBacConfig, async (req: Request, res: Response) => {
  ...
  try {
    if (endpoint === 'students') {
      const filters: any = {};
      if (req.query.grade_id) filters.grade_id = req.query.grade_id as string;
      if (req.query.active_only === 'true') filters.active_only = true;
      if (req.query.academic_year_id) filters.academic_year_id = req.query.academic_year_id as string;
 ...
 */
router.get('/term-grades', loadManageBacConfig, async (req: Request, res: Response) => {
  try {
    let apiKey: string;
    let baseUrl: string | undefined;
    ...
    let schoolId: number | undefined = req.manageBacConfig?.school_id ?? undefined;
    ...
    if (!allGrades) {
      if (gradeNumber != null && schoolId != null) {
        ...
        return res.status(400).json({
          error:
            'DP term-grade scope requires school_id on config (or run Get School Details). Pass all_grades=true to sync every class.',
        });
      }
...
router.post('/sync-to-rp', async (req: Request, res: Response) => {
  try {
    ...
    const result = await manageBacService.syncManageBacToRP(
      school_id != null ? String(school_id) : undefined,
      academic_year != null ? String(academic_year) : undefined
    );
    res.json({
      success: true,
