...
import {
  fisPreviousYearPeriodForYear,
  fisReportYearFromPeriod,
  isFisPreviousYearPeriod,
} from '../utils/fisPreviousYearPeriod.js';
...
export async function syncBsPreviousYearColumn(params: {
...
}): Promise<{ sourcePeriod: string; monthlyPeriodsUpdated: number; rowsCopied: number }> {
  ...
  const result = await executeProcedure<{
    monthly_periods_updated?: number;
    rows_copied?: number;
  }>('rp.usp_SyncFISPreviousYearColumn', {
    entity_code: entity,
...
export async function restoreBsPreviousYearAfterMonthlyRun(params: {
  ...
  if (isFisPreviousYearPeriod(params.asOfPeriod)) return;
  const sourcePeriod = fisPreviousYearPeriodForYear(fisReportYearFromPeriod(params.asOfPeriod));
  const entity = params.entityCode.trim().toUpperCase();
  const table =
    params.outputTable === 'new' ? 'rp.fis_report_output_new' : 'rp.fis_report_output';
