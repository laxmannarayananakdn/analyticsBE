...
export function isFisPhase4Enabled(): boolean {
  return process.env.FIS_PHASE4_ENABLED === 'true' || process.env.FIS_PHASE4_ENABLED === '1';
}

export function normalizeTbFileStatus(raw: string | null | undefined): FisFileStatus | null {
  if (!raw?.trim()) return null;
  const upper = raw.trim().toLowerCase();
  if (upper === 'final') return 'Final';
  if (upper === 'preliminary' || upper === 'prelim') return 'Preliminary';
  return null;
}

export function assertHomogeneousTbStatus(records: FinanceTrialBalanceRecord[]): FisFileStatus {
  const statuses = new Set<FisFileStatus>();
  for (const row of records) {
    const norm = normalizeTbFileStatus(row.status);
    if (!norm) {
      throw new Error(
        `Trial balance row has missing or invalid status "${row.status ?? ''}". ` +
...
export async function assertTbUploadAllowed(entityCode: string, period: string): Promise<void> {
  if (!isFisPhase4Enabled()) return;

  const entity = entityCode.trim().toUpperCase();
  const periodNorm = period.trim();

  const result = await executeQuery<{ entity_code: string }>(
  ...
  if (result.data?.length) {
    throw new Error(
      `Trial balance for ${entity} period ${periodNorm} is locked (Final Actual + Final Budget). ` +
...
export async function maybeLockEntityPeriod(
  entityCode: string,
  period: string,
  lockedBy: string | null
): Promise<boolean> {
  if (!isFisPhase4Enabled()) return false;

  const entity = entityCode.trim().toUpperCase();
  const periodNorm = period.trim();

  const cov = await executeQuery<{
    actual_file_status: string | null;
    budget_file_status: string | null;
    actual_upload_id: number | null;
    budget_upload_id: number | null;
  }>(
    `SELECT actual_file_status, budget_file_status, actual_upload_id, budget_upload_id
...
export async function resolveFileStatusForPeriod(
  entityCode: string,
  period: string
  ...
  // Actual and Budget are each optional alone. When only one side is loaded, generate using
  // that side's status and leave the missing side's columns as-is (report procs tolerate it).
  let actualStatus: FisFileStatus | null = null;
  if (uploads.actual) {
    actualStatus = await queryDominantTbStatus(entity, periodNorm, 'ACTUAL');
    if (!actualStatus) {
      throw new Error(
        `Cannot resolve Actual TB file status for ${entity} ${periodNorm}. ` +
          'Upload the Actual file with homogeneous Preliminary or Final status on every row.'
  ...
  if (actualStatus && budgetStatus && actualStatus !== budgetStatus) {
    throw new Error(
  ...
  const fileStatus = actualStatus ?? budgetStatus;
  if (!fileStatus) {
    throw new Error(`Cannot resolve TB file status for ${entity} period ${periodNorm}`);
  }
...
async function queryDominantTbStatus(
  entity: string,
  period: string,
  tbType: 'ACTUAL' | 'BUDGET'
): Promise<FisFileStatus | null> {
  const result = await executeQuery<{ file_status: string }>(
    `SELECT TOP 1
       CASE
...
export async function resolveRunLoggingContext(
  ...
  const actualStatus = uploads.actual
    ? await queryDominantTbStatus(entity, periodNorm, 'ACTUAL')
    : null;
  const budgetPeriod = uploads.budgetUsesFallback
    ? uploads.budgetSourcePeriod ?? periodNorm
    : periodNorm;
  const budgetStatus = uploads.budget
    ? await queryDominantTbStatus(entity, budgetPeriod, 'BUDGET')
    : null;
...
export async function getRunCalendar(entityCode?: string, year?: string): Promise<FisRunCalendarRow[]> {
  let query = `SELECT
  ...
  const result = await executeQuery<{
    ...
    budget_file_status: string | null;
    is_tb_locked: number;
    tb_locked_at: Date | null;
    nf_last_run_at: Date | null;
    nf_output_rows: number | null;
    nf_file_status: string | null;
    pl_last_run_at: Date | null;
    pl_output_rows: number | null;
    pl_file_status: string | null;
    bs_last_run_at: Date | null;
    bs_output_rows: number | null;
    bs_file_status: string | null;
    cf_last_run_at: Date | null;
    cf_output_rows: number | null;
    cf_file_status: string | null;
  }>(query, params);
...
export async function getPeriodCoverage(entityCode: string, period: string) {
  ...
  const result = await executeQuery<{
    ...
    `SELECT
     ...
     WHERE c.entity_code = @entity AND c.period = @period`,
    { entity, period: periodNorm }
  );
  if (result.error) throw new Error(result.error);
