...
export async function getAllDepartments(): Promise<Department[]> {
  const result = await executeQuery<Department>(
    `SELECT * FROM admin.Department ORDER BY Display_Order, Department_Name`
  );
 ...
 * Create department
 */
export async function createDepartment(createRequest: CreateDepartmentRequest): Promise<Department> {
...
export async function updateDepartment(
  departmentId: string,
  updateRequest: UpdateDepartmentRequest
): Promise<Department> {
