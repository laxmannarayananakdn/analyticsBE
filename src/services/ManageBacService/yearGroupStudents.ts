...
export async function getYearGroupStudents(
  ...
  try {
    ...
    if ((this as any).currentSchoolId && students.length > 0) {
...
export async function getAllYearGroupStudents(
  this: BaseManageBacService,
  apiKey: string,
  academicYearId?: string,
  termId?: string,
  ...
  try {
    ...
    let yearGroups = await databaseService.getYearGroupsForSchool((this as any).currentSchoolId);

    if (yearGroups.length === 0) {
      console.log('⚠️ No year groups found in database. Fetching year groups first...');
      await (this as any).getYearGroups(apiKey, baseUrl);
      const updatedYearGroups = await databaseService.getYearGroupsForSchool((this as any).currentSchoolId);
    ...
    for (const yearGroup of yearGroups) {
      try {
        ...
        const result = await (this as any).getYearGroupStudents(
          apiKey,
