...
export async function syncAllTermGrades(
  ...
  for (let ci = 0; ci < classIds.length; ci++) {
    ...
    if (!classDetails || !classDetails.start_term_id || !classDetails.end_term_id) {
      try {
        const apiClass = await (this as any).getClassById(apiKey, classId, baseUrl);
        if (apiClass) {
...
export async function syncTermGradesForYearGroup(
  this: BaseManageBacService,
  apiKey: string,
  yearGroupId: number
): Promise<void> {
  ...
  for (let i = 0; i < uniqueClassIds.length; i++) {
    ...
    for (const term of terms) {
      ...
      try {
        const termGradeResponse = await (this as any).getTermGrades(apiKey, classId, term.id);
