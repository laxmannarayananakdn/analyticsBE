...
import { databaseService, type AcademicYear as DBAcademicYear } from '../DatabaseService.js';
import type { AcademicYear } from '../../types/managebac.js';
...
export async function getAcademicYears(
  ...
  try {
    ...
    if ((this as any).currentSchoolId && response.data?.academic_years) {
      ...
      for (const [programKey, programInfo] of Object.entries(programsToProcess)) {
        ...
        for (const rawYear of rawYears) {
          ...
          if (rawYear.academic_terms && rawYear.academic_terms.length > 0) {
            const normalizedTerms = rawYear.academic_terms.map((term: any) => {
