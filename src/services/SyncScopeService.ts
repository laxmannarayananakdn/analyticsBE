 ...
 */
export async function getConfigsForScope(
  params: GetConfigsForScopeParams
): Promise<GetConfigsForScopeResult> {
  let mb: ManageBacConfig[];
  let nex: NexquareConfig[];
  ...
  if (params.configIdsMb?.length || params.configIdsNex?.length) {
    const [mbRes, nexRes] = await Promise.all([
      params.configIdsMb?.length
        ...
        : Promise.resolve([] as ManageBacConfig[]),
      params.configIdsNex?.length
        ? getConfigsByIds('nex', params.configIdsNex)
        : Promise.resolve([] as NexquareConfig[]),
    ]);
    mb = mbRes as ManageBacConfig[];
    nex = nexRes as NexquareConfig[];
    return { mb, nex };
  }
  if (params.all) {
 ...
 */
async function getConfigsByIds(
  source: 'mb' | 'nex',
  ids: number[]
): Promise<ManageBacConfig[] | NexquareConfig[]> {
  ...
  if (source === 'mb') {
    const result = await executeQuery<ManageBacConfig>(
      `SELECT id, api_token, base_url, school_name, school_id
  ...
  const result = await executeQuery<NexquareConfig>(
    `SELECT id, client_id, client_secret, domain_url, school_name, school_id
 ...
 */
async function getAllActiveConfigs(): Promise<GetConfigsForScopeResult> {
  const [mbResult, nexResult] = await Promise.all([
    executeQuery<ManageBacConfig>(
      `SELECT id, api_token, base_url, school_name, school_id
       FROM MB.managebac_school_configs
       WHERE is_active = 1
       ORDER BY country, school_name`
    ),
    executeQuery<NexquareConfig>(
      `SELECT id, client_id, client_secret, domain_url, school_name, school_id
       FROM NEX.nexquare_school_configs
       WHERE is_active = 1
       ORDER BY country, school_name`
 ...
 */
async function expandNodesWithDescendants(nodeIds: string[]): Promise<string[]> {
  ...
  const result = await executeQuery<{ Node_ID: string }>(query, params);
 ...
 */
async function getConfigsForNodes(
  nodeIds: string[]
): Promise<GetConfigsForScopeResult> {
  if (nodeIds.length === 0) return { mb: [], nex: [] };
  ...
  const [mbResult, nexResult] = await Promise.all([
    executeQuery<ManageBacConfig>(mbQuery, params),
    executeQuery<NexquareConfig>(nexQuery, params),
  ]);
