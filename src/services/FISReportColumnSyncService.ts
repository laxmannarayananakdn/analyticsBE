...
async function findOrCreateAutoInstance(
  reportTypeId: number,
  entityCode: string,
  uploadedBy: string
): Promise<number> {
  const name = autoInstanceName(entityCode);
  const existing = await executeQuery<{ instance_id: number }>(
    `SELECT instance_id FROM admin.fis_report_instances
     WHERE report_type_id = @reportTypeId AND instance_name = @name AND is_active = 1`,
    { reportTypeId, name }
  );
  ...
  const insert = await executeQuery<{ instance_id: number }>(
  ...
  if (!instanceId) throw new Error('Failed to create FIS report instance');

  await ensureInstanceCountry(instanceId, entityCode);
  return instanceId;
}

async function ensureInstanceCountry(instanceId: number, entityCode: string): Promise<void> {
  const check = await executeQuery<{ id: number }>(
    `SELECT id FROM admin.fis_instance_countries
     WHERE instance_id = @instanceId AND entity_code = @entityCode`,
    { instanceId, entityCode }
  );
...
async function renumberColumnOrder(instanceId: number): Promise<void> {
  const result = await executeQuery(
    `WITH ordered AS (
       SELECT column_id,
              ROW_NUMBER() OVER (
     ...
     INNER JOIN ordered o ON c.column_id = o.column_id`,
    { instanceId }
  );
  if (result.error) throw new Error(result.error);
}
...
async function upsertColumn(
  ...
  const existing = await executeQuery<{ column_id: number }>(
    `SELECT column_id FROM admin.fis_report_columns
     WHERE instance_id = @instanceId
       AND fiscal_year = @fiscalYear
       AND fiscal_month_from = @fiscalMonthFrom
       AND fiscal_month_to = @fiscalMonthTo`,
    params
  );
  if (existing.error) throw new Error(existing.error);

  const hasSourceCol = await executeQuery<{ has_col: unknown }>(
    `SELECT CASE WHEN COL_LENGTH('admin.fis_report_columns', 'source_file_name') IS NOT NULL THEN 1 ELSE 0 END AS has_col`
  );
  ...
  if (existing.data?.[0]?.column_id) {
    ...
    if (useSourceFileName) sets.push('source_file_name = @sourceFileName');

    const upd = await executeQuery(
      `UPDATE admin.fis_report_columns SET ${sets.join(', ')} WHERE column_id = @columnId`,
      { ...params, columnId: existing.data[0].column_id }
    );
  ...
  const ins = await executeQuery(
    `INSERT INTO admin.fis_report_columns (${columnList}) VALUES (${valueList})`,
    { ...params, columnOrder }
  );
  if (ins.error) throw new Error(ins.error);
  await renumberColumnOrder(instanceId);
 ...
 */
export async function syncFisReportColumnsFromTrialBalanceFile(
  fileName: string,
  uploadedBy: string
): Promise<SyncFisReportColumnsResult> {
  ...
  const instanceId = await findOrCreateAutoInstance(
    reportTypeId,
    parsed.entityCode,
    uploadedBy
  );
  ...
  console.log(
    `[FIS] Synced column ${parsed.periodYyyymm} for ${parsed.entityCode} → instance ${instanceId} (${columnAction}, "${parsed.columnLabel}")`
  );
 ...
 */
export async function repairFisFromCompletedTbUploads(): Promise<{
  uploadsProcessed: number;
  columnsSynced: number;
  errors: string[];
}> {
  const uploads = await executeQuery<{ file_name: string; uploaded_by: string }>(
    `SELECT u.file_name, u.uploaded_by
     FROM EF.Uploads u
  ...
  if (uploads.error) throw new Error(uploads.error);
  ...
  for (const row of uploads.data || []) {
    ...
    try {
      await backfillTrialBalanceEntityPeriod(parsed);
      const sync = await syncFisReportColumnsFromTrialBalanceFile(
        row.file_name,
        row.uploaded_by || AUTO_INSTANCE_CREATED_BY
      );
      if (sync.synced) columnsSynced += 1;
      else if (sync.message) errors.push(`${row.file_name}: ${sync.message}`);
    } catch (e) {
