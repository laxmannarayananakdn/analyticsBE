...
export async function verifyIbSchoolAvgVsGlobalAfterMbSync(
  input: IbSchoolAvgVerificationInput
): Promise<IbSchoolAvgVerificationResult> {
    ...
    executeQuery<{ cnt: number }>(
      `SELECT COUNT(*) AS cnt
       ...
       WHERE v.school_id = @school_id
         AND v.academic_year = @academic_year`,
      { school_id: schoolId, academic_year: academicYearRp }
    ),
    executeQuery<{ cnt: number }>(
      `SELECT COUNT(*) AS cnt
  ...
  if (viewRowCount === 0) {
    ...
    if (totalPointsCount === 0) {
      ...
      hints.push(
        'Total_Points exist but view is empty — check school is active in MB.managebac_school_configs or Node_School mapping'
      );
    }
...
export function logIbSchoolAvgVerification(
  schoolId: string,
  academicYearRp: string,
  result: IbSchoolAvgVerificationResult
): void {
  if (result.present) {
    console.log(
      `   [MB->RP] IB SchoolAvg vs Global: row present for school=${schoolId} academic_year=${academicYearRp} ` +
  ...
  console.warn(
    `   ⚠️ [MB->RP] IB SchoolAvg vs Global: no view row for school=${schoolId} academic_year=${academicYearRp}.${hintText}`
  );
}
