...
export const ADMIN_FOLDERS: { id: string; label: string }[] = [
...
export const ADMIN_ITEMS: { id: string; label: string; folder?: string }[] = [
...
export async function getSidebarItems(): Promise<SidebarItem[]> {
  const staticItems: SidebarItem[] = ADMIN_ITEMS.map((a) => ({
    id: a.id,
    label: a.label,
