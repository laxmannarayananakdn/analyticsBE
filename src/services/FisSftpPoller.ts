...
import { withFisSftp, type SftpFileEntry } from './FisSftpService.js';
...
async function processFileStub(file: SftpFileEntry): Promise<void> {
  console.log(
    `[FisSftp] Processing stub for ${file.name} (${file.size} bytes) – no-op until parsers are wired`
  );
}
    ...
    await withFisSftp(config, async (sftp) => {
      ...
      for (const file of files) {
        try {
          ...
          result.skipped += 1;
          console.error(
            `[FisSftp] Failed on ${file.name} (left in unprocessed; error folder not used yet):`,
            message
          );
        }
      }
