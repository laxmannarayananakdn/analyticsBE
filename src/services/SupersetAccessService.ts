 ...
 */
export async function getRlsRulesForUser(
  userEmail: string
): Promise<SupersetRlsRule[]> {
  const db = getPool();
  if (!db) {
    return [];
  }
  ...
  try {
    ...
    const result = await db.query(
      `SELECT DISTINCT rlsf.clause, rft.table_id AS dataset_id
    ...
    const rules: SupersetRlsRule[] = result.rows.map((r: { clause: string; dataset_id: number }) => ({
      clause: r.clause,
    ...
    if (rules.length > 0) {
      console.log(
        `🔒 RLS: Loaded ${rules.length} rule(s) for ${userEmail} (roles from Superset)`
      );
    }
    return rules;
  } catch (err) {
