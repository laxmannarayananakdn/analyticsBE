...
interface SupersetConfig {
  ...
  password: string;
  apiKey?: string;
  guestToken?: string; // Pre-generated guest token (JWT)
}
...
export class SupersetService {
   ...
   */
  private async getCsrfToken(bearerToken?: string): Promise<{ token: string; cookieHeader?: string }> {
    ...
    try {
      console.log(
        `🔐 Fetching CSRF token from ${this.config.baseUrl}/api/v1/security/csrf_token/`
      );

      const response = await fetch(
        `${this.config.baseUrl}/api/v1/security/csrf_token/`,
        { method: 'GET', headers: baseHeaders, redirect: 'follow' }
      );
      ...
      if (!response.ok) {
        ...
        if (response.status === 401 || response.status === 403) {
          throw new Error(
            `CSRF endpoint returned ${response.status}. This Superset instance may require Bearer token.`
          );
        }
      ...
      const cookieHeader = setCookies?.length
        ? setCookies.map((c: string) => c.split(';')[0].trim()).join('; ')
   ...
   */
  async getEmbeddedDashboardUuid(dashboardId: number): Promise<string> {
    try {
      const accessToken = await this.getAccessToken();
      const response = await fetch(
        `${this.config.baseUrl}/api/v1/dashboard/${dashboardId}/embedded`,
        {
          method: 'GET',
          headers: {
            Authorization: `Bearer ${accessToken}`,
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          throw new Error(
            `Dashboard ${dashboardId} does not have embedding enabled. Enable "Allow embedding" in Superset dashboard settings.`
          );
        }
        throw new Error(
          `Failed to get embedded dashboard UUID: ${response.status}. ${text}`
        );
      }
      ...
      if (!uuid) {
        throw new Error(
          `No embedded UUID returned for dashboard ${dashboardId}`
        );
      }
