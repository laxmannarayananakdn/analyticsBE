...
export class DatabaseService {
  ...
  async getStudentIdsInDpYearGroups(filters: {
    ...
    `;
    const result = await executeQuery<{ student_id: number }>(query, {
  ...
  async getMbTermGradeConfigTermIds(params: {
    ...
    const termsResult = await executeQuery<{ term_id: number; rubric_count: number }>(
      `SELECT term_id, COUNT(*) AS rubric_count
       FROM admin.mb_term_grade_rubric_config
       WHERE school_id = @school_id
    ...
    if (termsResult.error || !termsResult.data?.length) return null;
  ...
  async getMbTermGradeConfigRpAcademicYear(params: {
    ...
    const result = await executeQuery<{ academic_year_rp: string }>(
      `SELECT TOP 1 cfg.academic_year_rp
       FROM admin.mb_term_grade_rubric_config cfg
       WHERE cfg.school_id = @school_id
   ...
   */
  async getAcademicTermsByIds(termIds: number[]): Promise<AcademicTermRecord[]> {
    ...
    const result = await executeQuery<AcademicTermRecord>(
      `SELECT at.*
       FROM MB.academic_terms at
  ...
  async getDistinctClassesWithMemberships(filters?: {
    ...
    if (filters?.class_id != null) {
      const check = await executeQuery<{ class_id: number }>(
        'SELECT TOP 1 class_id FROM MB.class_memberships WHERE class_id = @class_id',
        { class_id: filters.class_id }
      );
      if (check.error || !check.data || check.data.length === 0) return [];
    ...
    if (filters?.grade_number != null && filters?.school_id != null) {
      ...
      `;
      const result = await executeQuery<{ class_id: number }>(query, {
        grade_number: filters.grade_number,
        school_id: filters.school_id,
      });
      if (result.error || !result.data) return [];
   ...
   */
  async bulkUpsertManageBacStudents(
    students: Student[],
    ...
    try {
      ...
      for (let i = 0; i < students.length; i += batchSize) {
        ...
        batch.forEach((record, index) => {
          const baseIndex = i + index;
          const parseDate = (d: Date | string | null | undefined): string | null => {
            if (!d) return null;
