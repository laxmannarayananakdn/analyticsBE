...
export class BaseNexquareService {
  ...
  protected tokenCache: Map<number, { token: string; expiresAt: number }> = new Map();
  protected currentSchoolId: string | null = null;

  /**
   * Get or refresh OAuth access token
   */
  protected async getAccessToken(config: NexquareConfig, forceRefresh: boolean = false): Promise<string> {
   ...
   */
  protected async makeFileRequest(
    endpoint: string,
    config: NexquareConfig,
    options: RequestInit = {},
    retryOnAuthError: boolean = true
   ...
   */
  protected async getSchoolSourcedId(schoolId: string): Promise<string | null> {
    try {
      ...
      console.warn(`⚠️  School "${schoolId}" not found in NEX.schools (sourced_id)`);
      return null;
    } catch (error: any) {
      console.error(`Error validating school ${schoolId}:`, error);
      return null;
    }
  }
