...
router.get('/runs/:id', async (req, res) => {
    try {
        ...
        const runResult = await executeQuery(`SELECT id, schedule_id, node_id, academic_year, status, started_at, completed_at,
              total_schools, schools_succeeded, schools_failed, triggered_by, error_summary, created_at
       FROM admin.sync_runs WHERE id = @id`, { id });
 ...
 */
router.get('/runs/:id/schools', async (req, res) => {
    try {
        const id = parseInt(req.params.id);
       ...
       ORDER BY id
       OFFSET @offset ROWS FETCH NEXT @limit ROWS ONLY`, { id, offset, limit });
        const countResult = await executeQuery(`SELECT COUNT(*) AS total FROM admin.sync_run_schools WHERE sync_run_id = @id`, { id });
...
router.post('/schedules', async (req, res) => {
    try {
        ...
        const result = await executeQuery(`INSERT INTO admin.sync_schedules (node_id, academic_year, cron_expression, endpoints_mb, endpoints_nex, load_rp_schema, build_student_assessments_by_academic_year, include_descendants, run_managebac, run_nexquare, created_by)
...
router.put('/schedules/:id', async (req, res) => {
    try {
        ...
        const result = await executeQuery(`UPDATE admin.sync_schedules SET ${updates.join(', ')} WHERE id = @id`, params);
        if (result.error) {
            return res.status(500).json({ error: result.error });
        }
        const selectResult = await executeQuery(`SELECT id, node_id, academic_year, cron_expression, endpoints_mb, endpoints_nex,
              ISNULL(load_rp_schema, 1) AS load_rp_schema,
              ISNULL(build_student_assessments_by_academic_year, 0) AS build_student_assessments_by_academic_year,
              ISNULL(run_managebac, 1) AS run_managebac,
              ISNULL(run_nexquare, 1) AS run_nexquare,
              include_descendants, is_active, updated_at
       FROM admin.sync_schedules WHERE id = @id`, { id });
...
router.post('/trigger', async (req, res) => {
    try {
        ...
        const insertResult = await executeQuery(`INSERT INTO admin.sync_runs (schedule_id, node_id, academic_year, status, started_at, total_schools, schools_succeeded, schools_failed, triggered_by)
       OUTPUT INSERTED.id
       VALUES (NULL, @nodeId, @academicYear, 'pending', SYSDATETIMEOFFSET(), 0, 0, 0, @triggeredBy)`, { nodeId: nodeIdStr, academicYear: academicYearStr, triggeredBy });
        ...
        setImmediate(async () => {
            try {
                ...
                await executeQuery(`UPDATE admin.sync_runs SET status = 'failed', completed_at = SYSDATETIMEOFFSET(), error_summary = @err WHERE id = @runId`, { err: err?.message || 'Sync failed', runId });
            }
            finally {
...
router.post('/runs/:id/cancel', async (req, res) => {
    try {
        ...
        const runResult = await executeQuery(`SELECT status FROM admin.sync_runs WHERE id = @id`, { id });
        ...
        await executeQuery(`UPDATE admin.sync_runs SET status = 'cancelled', completed_at = SYSDATETIMEOFFSET(), error_summary = @err WHERE id = @id`, { err: 'Cancelled (run was not in this process)', id });
        await executeQuery(`UPDATE admin.sync_run_schools SET status = 'skipped', completed_at = SYSDATETIMEOFFSET(), error_message = @msg
       WHERE sync_run_id = @id AND status IN ('pending', 'running')`, { msg: 'Cancelled', id });
        res.json({
            success: true,
