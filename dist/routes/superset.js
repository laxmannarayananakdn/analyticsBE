 ...
 */
router.post('/embed-token', authenticate, async (req, res) => {
    try {
        ...
        if (isSupersetAccessCheckEnabled()) {
            ...
            if (!accessResult.allowed) {
                console.warn(`🚫 Superset access denied: ${user.email} → dashboard ${dashboardIdString}, reason: ${accessResult.reason}`);
                const reasonMsg = accessResult.reason === 'user_not_found'
