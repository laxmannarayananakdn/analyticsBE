...
import { getAllFisCurrencyExchanges, getFisCurrencyExchangeById, getAvailableYears, createFisCurrencyExchange, updateFisCurrencyExchange, } from '../services/FisCurrencyExchangeService.js';
...
router.get('/', async (req, res) => {
    try {
        ...
        const year = yearParam != null && String(yearParam).trim() !== ''
...
router.get('/:id', async (req, res) => {
    try {
        ...
        if (!row)
            return res.status(404).json({ error: 'Currency exchange not found' });
        res.json(row);
    }
    catch (error) {
...
router.post('/', async (req, res) => {
    try {
        ...
        const { fromCurrency, currencyName, entityCodes, exchangeRate, exchangeType, effectiveFrom, effectiveTo, year, } = req.body;
        if (!fromCurrency ||
            !currencyName ||
            exchangeRate == null ||
            !exchangeType ||
            ...
            return res.status(400).json({
                error: 'fromCurrency, currencyName, exchangeRate, exchangeType, effectiveFrom, and year are required',
            });
        }
...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (isNaN(exchangeId))
            return res.status(400).json({ error: 'Invalid exchange ID' });
        const { currencyName, entityCodes, exchangeRate, exchangeType, effectiveFrom, effectiveTo, year, } = req.body;
        ...
        const msg = error instanceof Error ? error.message : 'Internal server error';
        if (msg === 'Currency exchange not found')
...
//# sourceMappingURL=fisCurrencyExchange.js.map