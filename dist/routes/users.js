...
import { syncUserToSuperset, syncSupersetUserActiveStatus, getSupersetRoleIdsForUser, isSupersetUserSyncEnabled, } from '../services/SupersetUserService.js';
...
router.post('/', async (req, res) => {
    try {
        ...
        const roleIds = Array.isArray(supersetRoleIds)
            ? supersetRoleIds.filter((r) => typeof r === 'number').map(Number)
            : [];
...
router.put('/:email', async (req, res) => {
    try {
        ...
        if (roleIds !== null && isSupersetUserSyncEnabled()) {
            try {
                const passwordHash = result.user.Auth_Type === 'Password' ? result.user.Password_Hash : null;
                await syncUserToSuperset(email, result.user.Display_Name, roleIds, result.user.Is_Active, passwordHash);
            }
            catch (supersetErr) {
        ...
        // Sync active status to Superset when isActive was updated (and roles weren't already synced)
            ...
            try {
                const passwordHash = result.user.Auth_Type === 'Password' ? result.user.Password_Hash : null;
                await syncSupersetUserActiveStatus(email, result.user.Is_Active, result.user.Display_Name, passwordHash);
            }
            catch (supersetErr) {
