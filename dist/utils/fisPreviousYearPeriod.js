/**
 * FIS Previous Year (period month 00) helpers.
 * e.g. 202600 = reporting year 2026 closing balances (Actual only).
 */
export function isFisPreviousYearPeriod(period) {
    const p = period.trim();
    return /^\d{6}$/.test(p) && p.slice(4, 6) === '00';
}
export function fisPreviousYearPeriodForYear(reportYear) {
    return `${reportYear}00`;
}
export function fisReportYearFromPeriod(period) {
    return parseInt(period.trim().slice(0, 4), 10);
}
/** Human label for processing UI, e.g. "Previous Year 2026". */
export function formatFisPreviousYearPeriod(period) {
    if (!isFisPreviousYearPeriod(period))
        return period;
    return `Previous Year ${period.slice(0, 4)}`;
}
//# sourceMappingURL=fisPreviousYearPeriod.js.map