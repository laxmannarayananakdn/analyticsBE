 ...
 */
export async function getHeadOfficeNodeId() {
    const result = await executeQuery(`SELECT TOP 1 Node_ID FROM admin.Node WHERE Is_Head_Office = 1`);
    if (result.error || !result.data || result.data.length === 0) {
        const fallback = await executeQuery(`SELECT TOP 1 Node_ID FROM admin.Node WHERE Parent_Node_ID IS NULL`);
