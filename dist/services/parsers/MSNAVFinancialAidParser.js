...
function normalizeJoinYear(value) {
...
export class MSNAVFinancialAidParser {
    ...
    async parseMSNAVFinancialAid(fileBuffer, skipInvalidRows = false) {
        ...
        try {
            ...
            const missingColumns = requiredColumns.filter(col => !Object.values(normalizedHeaders).includes(col));
            if (missingColumns.length > 0) {
                const columnError = validateRequiredColumns(headers, requiredColumns, headerMap);
                if (columnError) {
                    return {
                        valid: false,
                        errors: [columnError],
...
//# sourceMappingURL=MSNAVFinancialAidParser.js.map