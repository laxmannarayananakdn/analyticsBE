...
async function moveRemoteFile(sftp, file, destDir, label) {
    try {
        const move = await sftp.moveFileWithFallback(file.remotePath, destDir, file.name);
        if (move.usedFallback) {
            console.warn(`[FisSftp] Moved ${file.name} → ${label} as ${move.destFileName} (timestamp suffix; destination name already existed)`);
        }
...
async function processFinanceSftpFile(sftp, file, processedDir, errorDir, uploadedBy, result) {
    ...
    if (uploadResult.success) {
        ...
        if (move.moved) {
            ...
            console.log(`[FisSftp] Loaded ${uploadResult.rowCount} rows from ${file.name} (upload ${uploadResult.uploadId}) → processed${move.usedFallback ? ` as ${move.destFileName}` : ''}`);
        }
        else {
            result.skipped += 1;
            result.errors.push({
            ...
            console.warn(`[FisSftp] Loaded ${uploadResult.rowCount} rows from ${file.name} (upload ${uploadResult.uploadId}) but file remains in unprocessed: ${move.message}`);
        }
        return;
    }
    ...
    if (move.moved) {
        ...
        console.error(`[FisSftp] Failed ${file.name} (upload ${uploadResult.uploadId ?? 'n/a'}): ${message}; could not move to error`);
    }
}
/**
