...
export class ManageBacService {
     ...
     */
    async authenticate(apiKey, baseUrl) {
        try {
            ...
            const testResponse = await this.makeRequest(MANAGEBAC_ENDPOINTS.SCHOOL, apiKey, {}, baseUrl);
        ...
        try {
            ...
            if (error) {
                ...
                if (schoolData.enabled_programs && schoolData.enabled_programs.length > 0) {
                    ...
                    const programs = schoolData.enabled_programs.map((program) => ({
                        name: program.name,
                        code: program.code
                    }));
     ...
     */
    async getAcademicYears(apiKey, programCode, baseUrl) {
        try {
     ...
     */
    async getGrades(apiKey, academicYearId, baseUrl) {
        try {
     ...
     * Get all subjects
     */
    async getSubjects(apiKey, baseUrl) {
        try {
     ...
     * Get all teachers
     */
    async getTeachers(apiKey, filters, baseUrl) {
        try {
     ...
     */
    async getStudents(apiKey, filters, baseUrl) {
        try {
            ...
            if (this.currentSchoolId && students.length > 0) {
                ...
                const studentsForDb = students.map((student) => ({
     ...
     */
    async getClasses(apiKey, baseUrl) {
        try {
            const response = await this.makeRequest(MANAGEBAC_ENDPOINTS.CLASSES, apiKey, {}, baseUrl);
     ...
     */
    async getClassById(apiKey, classId) {
        try {
            const response = await this.makeRequest(`${MANAGEBAC_ENDPOINTS.CLASSES}/${classId}`, apiKey);
     ...
     */
    async getYearGroups(apiKey, baseUrl) {
        try {
            ...
            const response = await this.makeRequest(MANAGEBAC_ENDPOINTS.YEAR_GROUPS, apiKey, {}, baseUrl);
            const payload = response.data;
            const yearGroupsRaw = Array.isArray(payload?.year_groups)
                ? payload.year_groups
     ...
     */
    async getYearGroupStudents(apiKey, yearGroupId, academicYearId, termId, baseUrl) {
        try {
     ...
     */
    async getAllYearGroupStudents(apiKey, academicYearId, termId, baseUrl) {
        try {
     ...
     */
    async getMemberships(apiKey, userIds, academicYearId, termId, baseUrl, gradeNumber) {
        try {
            // Ensure school ID is set
            if (!this.currentSchoolId) {
                console.log('⚠️ School ID not set, fetching school details first...');
     ...
     */
    async getTermGrades(apiKey, classId, termId, baseUrl) {
        try {
     ...
     */
    async syncStudentsByGradesAndYearGroups(apiKey) {
        if (!this.currentSchoolId || this.studentsSyncedFromYearGroups) {
            return;
        }
        ...
        for (let i = 0; i < studentIdArray.length; i += batchSize) {
            ...
            const studentsForDb = await this.fetchStudentDetailsBatch(apiKey, batch, studentPlacement);
    ...
    async fetchYearGroupStudentIds(apiKey, yearGroupId) {
        try {
     ...
     */
    async fetchStudentDetailsBatch(apiKey, studentIds, placement) {
        const studentsForDb = [];
        for (const studentId of studentIds) {
            try {
     ...
     */
    async syncClassMembershipsForYearGroup(apiKey, yearGroupId) {
        if (!this.currentSchoolId) {
     ...
     */
    async syncTermGradesForYearGroup(apiKey, yearGroupId) {
        if (!this.currentSchoolId) {
            console.error('❌ No school context available');
            return;
        }
...
//# sourceMappingURL=ManageBacService.js.map