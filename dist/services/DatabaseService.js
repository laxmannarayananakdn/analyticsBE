...
export class DatabaseService {
    ...
    async upsertSchool(school) {
      ...
      `;
        ...
        `;
            ...
            const selectResult = await executeQuery(selectQuery, { id: school.id });
     ...
     */
    async upsertPrograms(programs, schoolId) {
     ...
     */
    async ensureGradesConstraintAllowsMultipleEntries() {
     ...
     */
    async upsertGrades(grades, schoolId) {
        ...
        for (const grade of grades) {
            try {
                ...
                const checkResult = await executeQuery(checkQuery, { uid: grade.uid });
     ...
     */
    async upsertSubjectGroups(groups, schoolId) {
        ...
        `;
          ...
          `;
                    ...
                    const selectResult = await executeQuery('SELECT * FROM MB.subject_groups WHERE id = @id', { id: group.id });
                    if (selectResult.error) {
     ...
     * Upsert subjects
     */
    async upsertSubjects(subjects, schoolId) {
        ...
        `;
          ...
          `;
                    ...
                    const selectResult = await executeQuery('SELECT * FROM MB.subjects WHERE id = @id', { id: subject.id });
                    if (selectResult.error) {
     ...
     * Upsert year groups
     */
    async upsertYearGroups(yearGroups, schoolId) {
        ...
        `;
          ...
          `;
                    ...
                    const selectResult = await executeQuery('SELECT * FROM MB.year_groups WHERE id = @id', { id: group.id });
                    if (selectResult.error) {
     ...
     */
    async getYearGroupsForSchool(schoolId) {
        const query = `
      SELECT id, school_id, name, short_name, program, grade, grade_number, created_at, updated_at
      FROM MB.year_groups
      WHERE school_id = @school_id
    `;
        const result = await executeQuery(query, { school_id: schoolId });
     ...
     */
    async getGradesForSchool(schoolId) {
        const query = `
     ...
     */
    async getAcademicTermsForSchool(schoolId) {
        const query = `
      SELECT at.*
    ...
    async getMbTermGradeConfigMbAcademicYear(params) {
        ...
        const cfgAyNorm = ayNormExpr.replace('%s', 'cfg.academic_year');
        const paramAyNorm = ayNormExpr.replace('%s', '@academic_year');
        const result = await executeQuery(`SELECT TOP 1 cfg.academic_year
       FROM admin.mb_term_grade_rubric_config cfg
       WHERE cfg.school_id = @school_id
         AND (
           cfg.academic_year = @academic_year
     ...
     */
    async getMbTermGradeConfigRpAcademicYear(params) {
        const ayHint = params.academic_year.trim();
        if (!ayHint)
            return null;
        const ayLike = `%${ayHint}%`;
        ...
        const cfgAyNorm = ayNormExpr.replace('%s', 'cfg.academic_year');
        const paramAyNorm = ayNormExpr.replace('%s', '@academic_year');
        const result = await executeQuery(`SELECT TOP 1 cfg.academic_year_rp
       FROM admin.mb_term_grade_rubric_config cfg
       WHERE cfg.school_id = @school_id
         AND cfg.academic_year_rp IS NOT NULL
         AND LTRIM(RTRIM(cfg.academic_year_rp)) <> ''
         AND (
           cfg.academic_year = @academic_year
           OR cfg.academic_year LIKE @academic_year_like
           OR @academic_year LIKE '%' + cfg.academic_year + '%'
       ...
       GROUP BY cfg.academic_year_rp
       ORDER BY cfg.academic_year_rp DESC`, {
            school_id: params.school_id,
            academic_year: ayHint,
            academic_year_like: ayLike,
        });
        if (result.error)
            return null;
        return result.data?.[0]?.academic_year_rp || null;
    }
    /**
    ...
    `;
      ...
      `;
        ...
        const params = { year_group_id: yearGroupId };
        if (limitStudents) {
    ...
    async getClassById(classId) {
      ...
      WHERE id = @class_id
    `;
        const result = await executeQuery(query, { class_id: classId });
     ...
     */
    async getAcademicTermsInRange(startTermId, endTermId) {
     ...
     */
    async upsertYearGroupStudent(yearGroupId, studentId) {
        try {
            const query = `
     ...
     */
    async getStudentsForYearGroup(yearGroupId) {
        const query = `
      SELECT s.*
      FROM MB.students s
      ...
      WHERE ygs.year_group_id = @year_group_id
    `;
        const result = await executeQuery(query, { year_group_id: yearGroupId });
        if (result.error || !result.data) {
            return [];
        }
        return result.data;
     ...
     * Upsert classes
     */
    async upsertClasses(classes, schoolId) {
        ...
        for (const classRecord of classes) {
            try {
                ...
                const checkResult = await executeQuery(checkQuery, { id: classRecord.id });
                if (checkResult.error) {
     ...
     */
    async upsertClassMemberships(memberships) {
     ...
     */
    async upsertAcademicYears(academicYears, schoolId, programCode) {
        ...
        `;
          ...
          `;
                    ...
                    const selectResult = await executeQuery('SELECT * FROM MB.academic_years WHERE id = @id', { id: year.id });
                    if (selectResult.error) {
     ...
     */
    async upsertAcademicTerms(terms, academicYearId) {
        ...
        `;
          ...
          `;
                    ...
                    const selectResult = await executeQuery('SELECT * FROM MB.academic_terms WHERE id = @id', { id: term.id });
                    if (selectResult.error) {
    ...
    async upsertStudents(students) {
        ...
        `;
          ...
          `;
                    ...
                    const selectResult = await executeQuery(selectQuery, { id: student.id });
                    if (selectResult.error) {
    ...
    async upsertTeachers(teachers, schoolId, onLog) {
        ...
        `;
                ...
                const resolveResult = await executeQuery(`SELECT id FROM MB.users WHERE id = @id OR email = @email`, { id, email: emailVal });
     ...
     * Upsert term grades
     */
    async upsertTermGrades(termGrades) {
    ...
    `;
        const [studentsResult, gradesResult, attendanceResult] = await Promise.all([
            ...
            executeQuery(averageGradeQuery),
            executeQuery(attendanceQuery)
        ]);
     ...
     */
    async getStudentVsClassAverage() {
        const query = `
      SELECT 
        tg.student_id,
        tg.class_id,
    ...
    `;
        ...
        classMap.forEach((classData) => {
            if (classData.students.length < 2)
                return; // Skip classes with only one student
     ...
     */
    async getFinancialAidDistribution() {
        const query = `
    ...
    `;
      ...
      `;
            const aidResult = await executeQuery(financialAidQuery);
     ...
     */
    async getNexquareSchool(sourcedId) {
        try {
            const query = `
        SELECT * FROM NEX.schools WHERE sourced_id = @sourced_id;
      `;
     ...
     */
    async deleteNexquareStudentAllocationsBySchoolAndYear(schoolSourcedId, academicYear) {
        if (!schoolSourcedId)
            return { deleted: 0, error: null };
        try {
            const query = academicYear != null && academicYear !== ''
     ...
     * When academicYear is null/empty, deletes rows where academic_year IS NULL.
     */
    async deleteNexquareStaffAllocationsBySchoolAndYear(schoolSourcedId, academicYear) {
        if (!schoolSourcedId)
            return { deleted: 0, error: null };
        try {
            const query = academicYear != null && academicYear !== ''
     ...
     */
    async deleteNexquareDailyPlansByDateRange(schoolSourcedId, startDate, endDate) {
        if (!schoolSourcedId)
            return { deleted: 0, error: null };
        try {
            const result = await executeQuery(`DELETE FROM NEX.daily_plans WHERE school_id = @schoolSourcedId AND plan_date >= @startDate AND plan_date <= @endDate; SELECT @@ROWCOUNT AS deleted;`, { schoolSourcedId, startDate, endDate });
            if (result.error)
                return { deleted: 0, error: result.error };
            const deleted = result.data?.[0]?.deleted ?? 0;
            return { deleted, error: null };
     ...
     */
    async deleteNexquareDailyAttendanceByDateRange(schoolSourcedId, startDate, endDate) {
        if (!schoolSourcedId)
            return { deleted: 0, error: null };
        try {
            const result = await executeQuery(`DELETE FROM NEX.daily_attendance WHERE school_id = @schoolSourcedId AND attendance_date >= @startDate AND attendance_date <= @endDate; SELECT @@ROWCOUNT AS deleted;`, { schoolSourcedId, startDate, endDate });
            if (result.error)
                return { deleted: 0, error: result.error };
            const deleted = result.data?.[0]?.deleted ?? 0;
            return { deleted, error: null };
     ...
     */
    async deleteNexquareStudentAssessmentsByYear(schoolSourcedId, academicYear) {
        if (!schoolSourcedId)
            return { deleted: 0, error: null };
        try {
            const result = await executeQuery(`DELETE FROM NEX.student_assessments WHERE school_id = @schoolSourcedId AND academic_year = @academicYear; SELECT @@ROWCOUNT AS deleted;`, { schoolSourcedId, academicYear });
            if (result.error)
                return { deleted: 0, error: result.error };
            const deleted = result.data?.[0]?.deleted ?? 0;
            return { deleted, error: null };
     ...
     */
    async deleteRPStudentAssessmentsByYear(schoolId, academicYear) {
        if (!schoolId)
            return { deleted: 0, error: null };
        try {
            const result = await executeQuery(`DELETE FROM RP.student_assessments WHERE school_id = @schoolId AND academic_year = @academicYear; SELECT @@ROWCOUNT AS deleted;`, { schoolId, academicYear });
            if (result.error)
                return { deleted: 0, error: result.error };
            const deleted = result.data?.[0]?.deleted ?? 0;
            return { deleted, error: null };
     ...
     */
    async bulkInsertDailyAttendance(records) {
        if (records.length === 0) {
     ...
     * Much faster than row-by-row inserts
     */
    async bulkInsertLessonAttendance(records) {
        if (records.length === 0) {
            return { inserted: 0, error: null };
        }
        const connection = await getConnection();
     ...
     */
    async bulkInsertDailyAttendanceViaTempTable(records) {
        if (records.length === 0) {
            return { inserted: 0, error: null };
        }
        const connection = await getConnection();
     ...
     * Uses parameterized queries for safety
     */
    async bulkInsertLessonAttendanceViaTempTable(records) {
        if (records.length === 0) {
            return { inserted: 0, error: null };
        }
        const connection = await getConnection();
     ...
     */
    async bulkInsertStudents(records) {
        if (records.length === 0) {
            return { inserted: 0, error: null };
        }
        const connection = await getConnection();
     ...
     */
    async bulkInsertClasses(records) {
        if (records.length === 0) {
            return { inserted: 0, error: null };
        }
        const connection = await getConnection();
     ...
     */
    async bulkInsertDailyPlans(records) {
        if (records.length === 0) {
            return { inserted: 0, error: null };
        }
        const connection = await getConnection();
     ...
     */
    async bulkInsertStudentAllocations(records) {
        if (records.length === 0) {
            return { inserted: 0, error: null };
        }
        const connection = await getConnection();
     ...
     */
    async bulkInsertStaffAllocations(records) {
        if (records.length === 0) {
            return { inserted: 0, error: null };
        }
        const connection = await getConnection();
