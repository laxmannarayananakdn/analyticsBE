 ...
 */
export async function getStaffAllocations(config, schoolId, academicYear) {
    try {
        ...
        if (recordsForInsert.length === 0) {
            console.log(`   ℹ️  No staff allocation records to insert`);
        ...
        if (schoolIdForDelete && yearsToDelete.length > 0) {
            ...
            for (const year of yearsToDelete) {
                const { deleted, error: deleteError } = await databaseService.deleteNexquareStaffAllocationsBySchoolAndYear(schoolIdForDelete, year);
                if (deleteError) {
                    console.warn(`⚠️  Failed to delete existing staff allocations (year: ${year ?? 'null'}): ${deleteError}`);
                }
                else if (deleted > 0) {
                    console.log(`   🗑️  Deleted ${deleted} existing staff allocation(s) for school/year ${year ?? 'null'}`);
                }
            }
        }
