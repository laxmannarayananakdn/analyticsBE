...
function buildEntityMap(rows) {
    ...
    for (const row of rows) {
        if (!map.has(row.currency_code))
            map.set(row.currency_code, []);
...
async function loadEntityMap() {
    const result = await executeQuery(`SELECT ce.currency_code, ce.entity_code, e.entity_name
...
function transform(row, entityMap) {
    return {
...
export async function getAvailableYears() {
    const result = await executeQuery(`SELECT DISTINCT [year] FROM admin.fis_currency_exchange ORDER BY [year] DESC`);
    if (result.error)
        throw new Error(result.error);
    return (result.data || []).map((r) => r.year);
}
export async function getAllFisCurrencyExchanges(year) {
    const params = {};
    let where = '';
    if (year != null) {
    ...
    const [result, entityMap] = await Promise.all([
        executeQuery(`SELECT ${SELECT_COLS}
       FROM admin.fis_currency_exchange e
       LEFT JOIN admin.fis_currency c ON c.currency_code = e.from_currency
       ${where}
       ORDER BY e.[year] DESC, e.from_currency, e.exchange_type, e.effective_from DESC`, params),
        loadEntityMap(),
    ]);
    if (result.error)
        throw new Error(result.error);
...
export async function getFisCurrencyExchangeById(exchangeId) {
    const [result, entityMap] = await Promise.all([
        executeQuery(`SELECT ${SELECT_COLS}
       FROM admin.fis_currency_exchange e
       LEFT JOIN admin.fis_currency c ON c.currency_code = e.from_currency
       WHERE e.exchange_id = @exchangeId`, { exchangeId }),
        loadEntityMap(),
    ]);
    if (result.error)
        throw new Error(result.error);
...
async function validateEntityCodes(entityCodes) {
    const unique = [...new Set(entityCodes.map((c) => c.trim().toUpperCase()).filter(Boolean))];
    if (unique.length === 0)
        return;
    for (const code of unique) {
        const exists = await executeQuery(`SELECT entity_code FROM admin.fis_entity WHERE entity_code = @entityCode`, { entityCode: code });
        if (exists.error)
            throw new Error(exists.error);
        if (!exists.data?.length)
            throw new Error(`Entity not found: ${code}`);
    }
}
async function assertEntitiesAvailableForCurrency(currencyCode, entityCodes) {
    const unique = [...new Set(entityCodes.map((c) => c.trim().toUpperCase()).filter(Boolean))];
    if (unique.length === 0)
        return;
    for (const entityCode of unique) {
        const conflict = await executeQuery(`SELECT currency_code FROM admin.fis_currency_entity
       WHERE entity_code = @entityCode AND currency_code <> @currencyCode`, { entityCode, currencyCode });
        if (conflict.error)
...
async function upsertCurrencyMaster(currencyCode, currencyName, entityCodes, user) {
    const code = normalizeCurrency(currencyCode);
    const name = currencyName.trim();
    if (!code)
        throw new Error('fromCurrency is required');
    if (!name)
        throw new Error('currencyName is required');
    ...
    const existing = await executeQuery(`SELECT currency_code FROM admin.fis_currency WHERE currency_code = @currencyCode`, { currencyCode: code });
    if (existing.error)
        throw new Error(existing.error);
    if (existing.data?.length) {
        const update = await executeQuery(`UPDATE admin.fis_currency SET currency_name = @currencyName, updated_by = @updatedBy
       WHERE currency_code = @currencyCode`, { currencyCode: code, currencyName: name, updatedBy: user });
        if (update.error)
            throw new Error(update.error);
    }
    else {
        const insert = await executeQuery(`INSERT INTO admin.fis_currency (currency_code, currency_name, created_by)
       VALUES (@currencyCode, @currencyName, @createdBy)`, { currencyCode: code, currencyName: name, createdBy: user });
        if (insert.error)
            throw new Error(insert.error);
    }
    if (normalizedEntities) {
        const del = await executeQuery(`DELETE FROM admin.fis_currency_entity WHERE currency_code = @currencyCode`, { currencyCode: code });
        if (del.error)
            throw new Error(del.error);
        for (const entityCode of normalizedEntities) {
            const ins = await executeQuery(`INSERT INTO admin.fis_currency_entity (currency_code, entity_code, created_by)
         VALUES (@currencyCode, @entityCode, @createdBy)`, { currencyCode: code, entityCode, createdBy: user });
            if (ins.error)
                throw new Error(ins.error);
        }
    }
}
export async function createFisCurrencyExchange(req) {
    ...
    if (!fromCurrency)
        throw new Error('fromCurrency is required');
    if (fromCurrency === TO_CURRENCY)
        throw new Error('fromCurrency cannot be USD');
    ...
    await upsertCurrencyMaster(fromCurrency, req.currencyName, req.entityCodes ?? [], req.createdBy || 'system');
    const result = await executeQuery(`INSERT INTO admin.fis_currency_exchange (
...
export async function updateFisCurrencyExchange(exchangeId, req) {
    ...
    if (req.currencyName !== undefined || req.entityCodes !== undefined) {
        await upsertCurrencyMaster(existing.fromCurrency, req.currencyName ?? existing.currencyName ?? existing.fromCurrency, req.entityCodes ?? existing.entities.map((e) => e.entityCode), req.updatedBy || 'system');
    }
    ...
    const result = await executeQuery(`UPDATE admin.fis_currency_exchange SET
...
//# sourceMappingURL=FisCurrencyExchangeService.js.map