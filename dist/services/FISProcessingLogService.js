...
function mapUploadStatus(status) {
...
export async function listSftpPullLog(params) {
  ...
  `;
    const sqlParams = { limit, sftpUser };
    ...
    return (result.data || [])
        ...
        const isTb = row.file_type_code === 'FIN_TB_ACTUAL' || row.file_type_code === 'FIN_TB_BUDGET';
        return {
            uploadId: row.id,
            fileName: row.file_name,
            fileTypeCode: row.file_type_code,
            category: isTb ? 'TB' : isDic ? 'DIC' : 'OTHER',
        ...
        .filter((row) => {
        if (entityFilter && row.entityCode !== entityFilter)
            return false;
...
//# sourceMappingURL=FISProcessingLogService.js.map