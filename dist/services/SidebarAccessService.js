 ...
 */
export async function setUserSidebarAccess(email, itemIds, createdBy) {
    const userCheck = await executeQuery(`SELECT User_ID FROM admin.[User] WHERE User_ID = @email`, { email });
    ...
    for (const itemId of itemIds) {
        const r = await executeQuery(`INSERT INTO admin.user_sidebar_access (User_ID, Item_ID, Created_At, Created_By) VALUES (@email, @itemId, SYSDATETIMEOFFSET(), @createdBy)`, { email, itemId, createdBy });
        if (r.error)
 ...
 */
export async function getSidebarAccessMatrix() {
    const [itemsResult, usersResult, permResult] = await Promise.all([
        getSidebarItems(),
        executeQuery(`SELECT User_ID, Display_Name FROM admin.[User] WHERE Is_Active = 1 ORDER BY Display_Name, User_ID`),
        executeQuery(`SELECT User_ID, Item_ID FROM admin.user_sidebar_access`),
    ]);
...
//# sourceMappingURL=SidebarAccessService.js.map